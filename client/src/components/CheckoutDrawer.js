...
export default function CheckoutDrawer({
  ...
  const onDelete = async (items, index) => {
    let id = items.UUID;
    axios
      .delete(`${API.website}/shopping-cart/${id}/${user_dod}`)
      .then((res) => {
        if (res.status === 200) {
          fetchNewShoppingCart();
  ...
  const changeItemQuantity = async (items, index) => {
    ...
    axios
      .patch(
        `${API.website}/shopping-cart-quantity/${id}/${user_dod}/${ogWarehouse}`,
        newQuantity
      )
      .then((res) => {
        if (res.status === 200) {
  ...
  const subtractFromInventory = async (items, index) => {
    flatCart.forEach((items, index) => {
      ...
      console.log(
        `id: ${id}, ogWarehouse: ${ogWarehouse}, ItemCount: ${cartItemCount}, items.Quantity: ${items.Quantity}`
      );
      axios
        ...
        .then((res) => {
          ...
          axios.patch(
            `${API.website}/inventorysubtractcount/${id}/${newCount}/${user_dod}/${ogWarehouse}`,
            newQuantity
          );
          axios.patch(`${API.website}/removeitemfromcart/${id}/${user_dod}`);
          if (res.status === 200) {
            fetchNewShoppingCart();
            // fetchInventory();
          }
  ...
  const addToInventoryCount = async (items, index) => {
    flatCart.forEach((items, index) => {
      ...
      axios.patch(
        `${API.website}/inventoryaddcount/${id}/${newCount}/${user_dodid}/${user_dod}`,
        newQuantity
      );
      axios
  ...
  const addToSelectedWarehouse = async (items, index) => {
    flatCart.forEach((items, index) => {
      ...
      let ogWarehouse = items.Original_warehouse;
      axios.patch(
        `${API.website}/addToSelectedWarehouse/${selectedWarehouse}`,
        items
      );
  ...
  const availableWarehouses = adminWarehouses?.map((warehouse) =>
    warehouse?.warehouse_access?.map((warehouses) => {
      return warehouses;
    })
  );
  ...
  const cart = newShoppingCart?.map((item, index) =>
  ...
  const clearCart = async () => {
    ...
    axios
      .patch(`${API.website}/clear-shopping-cart/${user_dod}`)
      .then((res) => {
        if (res.status === 200) {
          fetchNewShoppingCart();
          window.location.reload();
            ...
            <List>
                                        ...
                                        onChange={(e) => {
                                          if (
                                            ...
                                            setNewQuantity({
                                              ...newQuantity,
                                              Quantity: 0,
                                              uuidDate: items.UUIDfetcha,
                                              UUID: items.UUID,
                                              Original_warehouse:
                                                items.Original_warehouse,
                                            });
                                          } else {
                                            {
                                              if (
              ...
              <ListItem sx={{ display: "flex", flexDirection: "column" }}>
                {cartLength >= 1 ? (
                  ...
                  <div>
                    <Box sx={{ mt: 10 }}>
                      <Box sx={{ ml: 5 }}>Your cart is currently empty.</Box>
                      <Box>Add to the cart to issue or return items.</Box>
                    </Box>
                  </div>
                )}
              </ListItem>
